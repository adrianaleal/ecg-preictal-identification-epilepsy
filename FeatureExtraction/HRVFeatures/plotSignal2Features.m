function [] = plotSignal2Features(feat_names, patient_name)
...
% - patient_name (cell): patient index name
...
disp(['Patient ' patient_name])
patFolderName = ['pat_' patient_name];
...
for ss = 1:n_seizures_pat
    ...
    hrv_features = load([patFolderName '_seizure' num2str(ss)], 'hrv_features');
    hrv_features = hrv_features.hrv_features;
    
    time_vec_original = vertcat(hrv_features.time);
    
    % load the ECG signal, the R-peaks and the RR interval series
    load(['pat_' patient_name '_seizure' num2str(ss), '_save2FigurePlotECG.mat'], ...
        'save2FigurePlotECG')
    ...
    title(['Patient ' patient_name ', Seizure ' num2str(ss)])
    ...
    if plotTimeMin
        ...
        if plotTimeMin
            if plotTimeMinReverse
                xline(win1(1)-duration_seiz_min, 'k--');
                xline(win1(2)-duration_seiz_min, 'k--');
                xline(win2(1)-duration_seiz_min, 'k--');
                xline(win2(2)-duration_seiz_min, 'k--');
            else
                xline(win1(1), 'k--');
                xline(win1(2), 'k--');
                xline(win2(1), 'k--');
    ...
    axis tight, grid minor
    legend(replaceFeatureNames(feat_names))
    if plotTimeMin
        set(gca,'XTick', vec_time_min)
        set(gca,'XTickLabel', num2cell(vec_time_min_label))
