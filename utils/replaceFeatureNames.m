...
if ~isempty(index)
    for ll = 1:numel(index)
        feat_names2analyse(index(ll)) = {[strrep(feat_names2analyse{index(ll)},'SD','SD$_') '$']};
    end
end
...
if ~isempty(index)
    for ll = 1:numel(index)
        feat_names2analyse(index(ll)) = {[strrep(feat_names2analyse{index(ll)},'SD1toSD2','SD$_1$/SD$_2') '$']};
    end
end
