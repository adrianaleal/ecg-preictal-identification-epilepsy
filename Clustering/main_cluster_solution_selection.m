...
time_vec_original = vertcat(seizure_struct(:).time_vec);
time_vec_original = vertcat(time_vec_original{:});
percent_noise_original = vertcat(seizure_struct(:).percent_noise);
percent_noise_original = vertcat(percent_noise_original{:});
...
%% Perform cluster solution selection
...
results_folder_path = fullfile(cd, 'ResultsSelection');

prototype_patient_clustering_by_seizure(feat_names2analyse, ...
    feat_comb, patients_name, seizure_names_separated, results_folder_path, ...
