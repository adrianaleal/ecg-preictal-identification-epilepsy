function h1 = plotClusterSolution(data2cluster, clusteringSolution, ...
    centroids, distances, dimension, plotOnlyClustering, feat_names, ...
    time, colors, markers)

if isempty(feat_names)
    feat_names = strcat('feat', cellstr(num2str((1:dimension)')));
end
    ...
    if dimension==3
        for ll = 1:ncluster
            ...
            if isempty(markers)
                ...
                h1(ll) = scatter3(data2cluster(clusteringSolution==n_cluster_values(ll),1), ...
                    data2cluster(clusteringSolution==n_cluster_values(ll),2),...
                    data2cluster(clusteringSolution==n_cluster_values(ll),3),[],colors(ll,:));
                %         h(ll) = scatter3(data2cluster(clusteringSolutions==ll,1),data2cluster(clusteringSolutions==ll,2),...
                %             data2cluster(clusteringSolutions==ll,3),marker_size(ll),colors(ll,:),marker(ll));
                hold on
        ...
        for ll = 1:ncluster
            colors = jet(ncluster);
            h1(ll) = scatter(data2cluster(clusteringSolution==n_cluster_values(ll),1), ...
                data2cluster(clusteringSolution==n_cluster_values(ll),2),[],colors(ll,:));
            %         h(ll) = scatter3(data2cluster(clusteringSolutions==ll,1),data2cluster(clusteringSolutions==ll,2),...
            %             data2cluster(clusteringSolutions==ll,3),marker_size(ll),colors(ll,:),marker(ll));
            hold on
            if ~isempty(centroids)
...
%         time = 5:5:240;% linspace(240,0,size(data2cluster,1));
        ...
        for ll = 1:ncluster
            ...
            %         h(ll) = scatter3(data2cluster(clusteringSolutions==ll,1),data2cluster(clusteringSolutions==ll,2),...
            %             data2cluster(clusteringSolutions==ll,3),marker_size(ll),c(clusteringSolutions==ll),marker(ll));
            %         hold on
            %         if ~isempty(centroids)
            %             h2(ll) = plot3(centroids(ll,1),centroids(ll,2),centroids(ll,3),'kx','MarkerSize',15,'LineWidth',3);
            %         end
        ...
        for ll = 1:ncluster
            ...
            h1(ll) = scatter3(data2cluster(clusteringSolution==n_cluster_values(ll),1), ...
                data2cluster(clusteringSolution==n_cluster_values(ll),2),...
                data2cluster(clusteringSolution==n_cluster_values(ll),3),[],map(ll,:),'filled');
            %         h(ll) = scatter3(data2cluster(clusteringSolutions==ll,1),data2cluster(clusteringSolutions==ll,2),...
            %             data2cluster(clusteringSolutions==ll,3),marker_size(ll),colors(ll,:),marker(ll));
            hold on
            if ~isempty(centroids)
...
%             caxis([min(n_cluster_values) max(n_cluster_values)]);
    ...
    elseif dimension==2 && ncluster<7
        ...
        ax1 = subplot(131);
        for ll = 1:ncluster
            h(ll) = scatter(data2cluster(clusteringSolution==n_cluster_values(ll),1), ...
                data2cluster(clusteringSolution==n_cluster_values(ll),2),...
                marker_size(ll),time(clusteringSolution==n_cluster_values(ll)),marker(ll)); hold on
            %         if ~isempty(centroids)
            %             h2(ll) = plot(centroids(ll,1),centroids(ll,2),'kx','MarkerSize',15,'LineWidth',3);
            %         end
        end
        ...
        distances = pdist2(clusteringSolution,clusteringSolution)+1;
        imagesc(distances,'CDataMapping','scaled');
        colormap(ax3,colors)
        caxis([1 ncluster]);
        time_axis = linspace(240,0,7);
