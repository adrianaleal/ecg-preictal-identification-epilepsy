function [SD1, SD2, SDRR] = poincare_plot(RR_interval_signal_segment,plot_poincare)

% Read Shaffer2017 for interpretation of the results


RRi = RR_interval_signal_segment(1:end-1); % RR(i)
RRii = RR_interval_signal_segment(2:end); % RR(i+1)
...
%     sd2_factor = sd1_factor;
    ...
    legend([h_sd1,h_sd2], {['$SD_1 = ' num2str(round(SD1*1e4)/1e4) '$ (short-term HRV)'], ...
        ['$SD_2 = ', num2str(round(SD2*1e4)/1e4) '$ (long-term HRV)']});
